-- Postgres Lab: the "shop" dataset (~2.35M rows), identical to the in-browser lab.
-- Usage:  psql -h localhost -U postgres -v ON_ERROR_STOP=1 -f seed.sql
-- Takes ~20-60 s on a laptop. Safe to re-run: it drops and recreates the lab tables.
\timing on
...
FROM (
  SELECT g,
         (ARRAY['aarav','priya','liam','olivia','noah','emma','mateo','sofia','kenji','yuki','amara','chidi','lukas','hanna','rohan','ananya','diego','lucia','omar','layla','ethan','ava','arjun','isha','felix','clara','tomas','ines','kofi','zara','leo','mia','ravi','sana','hugo','elena','sam','nora','jin','maya'])[1 + floor(random() * 40)::int] AS fn,
         (ARRAY['sharma','patel','smith','johnson','garcia','silva','tanaka','sato','okafor','adeyemi','muller','schmidt','kumar','iyer','lopez','martin','nguyen','kim','brown','wilson','rossi','dubois','novak','haddad','mensah','larsen','costa','reddy','chen','wang','singh','khan','ali','murphy','walsh','fischer','weber','ito','moreau','santos'])[1 + floor(random() * 40)::int]  AS ln,
...
WITH picks AS MATERIALIZED (
  SELECT (1 + floor((random() ^ 2) * 50000))::bigint AS customer_id,
         random() AS r1, random() AS r2, random() AS r3, random() AS r4, random() AS r5,
         floor((random() ^ 1.6) * 5000)::int AS pbase
...
SELECT o.id AS order_id,
       1 + ((o.pbase + k * 997) % 5000) AS product_id,
...
SELECT CASE WHEN ra < 0.25 THEN NULL ELSE (1 + floor(rb * 50000))::bigint END,
       et, ts,
       CASE et
         WHEN 'page_view'   THEN jsonb_build_object('path', '/p/' || (1 + floor(rc * 5000)::int), 'device', dev, 'ms', 40 + floor((rd ^ 3) * 4000)::int)
         ...
         WHEN 'add_to_cart' THEN jsonb_build_object('product_id', 1 + floor(rc * 5000)::int, 'qty', 1 + floor(rd * 3)::int)
       ...
       END
FROM (
  SELECT timestamptz '2026-01-01' + ((g - 1)::float8 / 600000) * interval '181 days' + random() * interval '20 seconds' AS ts,
  ...
  FROM (SELECT g, random() AS r FROM generate_series(1, 600000) g) x
) s
ORDER BY ts;
...
FROM (
  SELECT product_id, customer_id, rating, t, created_at,
         ...
         END AS b
  FROM (
    SELECT 1 + floor((random() ^ 1.8) * 5000)::int AS product_id,
           1 + floor(random() * 50000)::int AS customer_id,
    ...
    FROM (SELECT random() AS rr FROM generate_series(1, 100000)) x
  ) y
) z
ORDER BY created_at;
...
-- Extensions used by the lessons (pg_stat_statements also needs shared_preload_libraries = pg_stat_statements)
